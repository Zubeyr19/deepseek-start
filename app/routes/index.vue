...
<template>
  <div class="flex flex-col min-h-screen bg-gray-900">
    ...
    <!-- System prompt area - styled exactly like React version -->
    <div class="p-4 container mx-auto max-w-4xl space-y-4">
      <label for="premise">
        Premise:
        <textarea 
          id="premise"
    ...
    <!-- Chat area -->
    <div class="flex-1 p-4 container mx-auto max-w-4xl space-y-4 pb-32">
    ...
    <!-- Input area - matched with React styling -->
    <div class="fixed bottom-0 left-0 right-0 p-4 bg-gray-800 border-t border-gray-700">
      <form @submit.prevent="handleSubmit" class="container mx-auto max-w-4xl">
        <div class="flex gap-2">
          <div class="flex-1 relative">
            <MessageSquare class="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
            <input
              type="text"
              class="flex-1 bg-gray-900 border-gray-700 text-gray-100 pl-10 w-full rounded-lg py-2"
              v-model="input"
              :disabled="loading"
          ...
          <button
            ...
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center justify-center"
          >
...
Keep your answer simple and straight forward.`);
    ...
    const fetchInternalData = async (dataType) => {
      try {
       let endpoint; 
